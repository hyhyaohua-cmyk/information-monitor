...
function reports(){{const q=search.value.trim().toLowerCase(),cat=category.value,sid=site.value,rid=run.value;const rows=data.reports.filter(x=>(!cat||x.category===cat)&&(!sid||x.site_id===sid)&&(!rid||x.run_id===rid)&&(!q||`${{x.title_zh}} ${{x.title}} ${{x.url}} ${{x.site_name}}`.toLowerCase().includes(q))).map(x=>[`<div class="title"><a href="${{esc(x.url)}}" target="_blank" rel="noopener noreferrer">${{esc(x.title_zh||x.title||'标题暂不可用')}}</a></div>${{x.title&&x.title!==x.title_zh?`<div>${{esc(x.title)}}</div>`:''}}<div class="url muted"><a href="${{esc(x.url)}}" target="_blank" rel="noopener noreferrer">${{esc(x.url)}}</a></div>`,`<span>${{esc(x.site_name)}}</span><br><span class="pill">${{esc(x.category)}}</span>`,when(x.created_at)]);reportBody.innerHTML=table(['中文标题 / 原标题 / 链接','来源','发现时间'],rows)}}
